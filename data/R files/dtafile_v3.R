...
combine_multiselect_with_label_values <- function(data, prefix) {
  ...
  col_labels <- sapply(cols, function(col) {
    ...
    if (length(label_for_1) > 0) label_for_1 else "Unknown"
  })
...
for (prefix in prefixes_to_combine) {
  ces2021_filtered_dta_v3 <- combine_multiselect_with_label_values(ces2021_filtered_dta_v3, prefix)
}
...
ces2021_filtered_dta_v3 <- ces2021_filtered_dta_v3 %>%
  mutate(across(everything(), convert_to_labels))

# Replace column names with labels from loaded JSON (ces2021_label_map_json)
names(ces2021_filtered_dta_v3) <- sapply(names(ces2021_filtered_dta_v3), function(var) {
  label <- ces2021_label_map_json[[var]]
