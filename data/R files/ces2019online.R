...
var_labels <- sapply(ces2019online_filtered_dta, function(x) attr(x, "label"))

# For each prefix group like "cps21_lead_int"
for (prefix in prefixes_to_combine) {
  ces2019online_filtered_dta <- combine_multiselect_overwrite_first_with_label(ces2019online_filtered_dta, prefix)
}

library(haven)

names(ces2019online_filtered_dta) <- sapply(names(ces2019online_filtered_dta), function(col) {
  label <- attr(ces2019online_filtered_dta[[col]], "label")
...
library(jsonlite)
write_json(ces2019online_filtered_dta, 
           "C:/Users/jchen/YouthEmploymentEducationProject/react-d3/public/ces2019_data.json", pretty = TRUE)